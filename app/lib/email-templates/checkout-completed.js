...
export function generateCheckoutCompletedEmail(session) {
...
<head>
  ...
  <style>
    ...
    .item-list { margin: 24px 0; }
    .item { padding: 12px 0; border-bottom: 1px solid #e2e8f0; }
    .item:last-child { border-bottom: none; }
    .item-name { font-weight: 600; color: #0F172A; }
    .item-price { color: #64748B; font-size: 14px; margin-top: 4px; }
  </style>
</head>
<body>
  <div class="container">
    ...
    <div class="content">
      ...
      ${lineItems.length > 0 ? `
      <div class="item-list">
        <div style="font-size: 14px; font-weight: 600; color: #0F172A; margin-bottom: 16px;">Détails de votre commande :</div>
        ${lineItems.map(item => `
          <div class="item">
            <div class="item-name">${item.plan?.nickname || item.description || 'Plan d\'adhésion'}</div>
            <div class="item-price">${((item.amount || 0) / 100).toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}</div>
          </div>
        `).join('')}
      </div>
      ` : ''}
